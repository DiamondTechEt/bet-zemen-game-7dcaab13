...
export function GameForm({ existing, onDone }: { existing?: any; onDone?: () => void }) {
  const { user } = useAuth();
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) { console.error("game save error", error); toast.error(error.message || "ማስቀመጥ አልተሳካም"); setBusy(false); return; }
      toast.success("ተቀምጧል");
      onDone?.();
    } catch (err: any) {
